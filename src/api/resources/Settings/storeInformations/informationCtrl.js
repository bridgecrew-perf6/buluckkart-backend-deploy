...
export default {
    ...
    async addstoreInformationSettings(req, res, next) {
        ...
        try {
            const {StoreName,StoreContactPerson,StoreContactNumber, StoreEmail,Location, City, State,Country,Timezone, Zipcode, Currency, AppShareLink, AndroidShareLink ,  phoneShareLink, ShowCurrency, UploadStoreLogo,ShowUploadAppIcon,AboutUs}= req.body;
            const storeInformationSettingsDetails = await db.storeInformationSettings.create({
              ...
              UploadStoreLogo:req.file ? req.file.location : '',
...
async getstoreInformationSettings(req, res) {
    try {
        const storeInformationSettingsDetails = await db.storeInformationSettings.findOne({ where: { id: req.params.id } });
        res.status(200).send({
            status: 200,
    ...
    async updatestoreInformationSettings(req, res){
        try {
            ...
            const updatestoreInformationSettings =await db.storeInformationSettings.update({
              ...
              phoneShareLink:phoneShareLink,
              ShowCurrency:ShowCurrency,
              UploadStoreLogo:req.file ? file.Location: '',
              ShowUploadAppIcon:req.file  ?file.Location:'',
              AboutUs:AboutUs
        },
